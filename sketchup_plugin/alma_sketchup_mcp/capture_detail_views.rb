...
# Modified 2026-09-12: use Ruby 2.5-compatible hash construction for SketchUp 2020.
...
module AlmaSketchupMCP
  ...
  def detail_capture_facing_preservation_signature(model, records)
    ...
    enrolled = records.map { |record| [record[:entity].object_id, record] }.to_h
    ...
    raise 'detail_capture_camera_facing_guard_incomplete' unless state['complete'] == true
    source = %w[totals materials native_appearance tags scenes section_planes classification_schemas component_definition_summaries image_references].map { |key| [key, model_snapshot[key]] }.to_h
  ...
  def detail_scene_restore_plan(model, validate = true)
    ...
    drawing_elements = collections.flat_map(&:to_a).select { |entity| entity.respond_to?(:hidden?) && entity.respond_to?(:hidden=) }.uniq
    {
      ...
      rendering: model.rendering_options.keys.map { |key| [key, model.rendering_options[key]] }.to_h,
      shadow: model.shadow_info.keys.map { |key| [key, model.shadow_info[key]] }.to_h,
