...
module AlmaSketchupMCP
  ...
  def set_style(model, operation)
    ...
    state['face_style'] = material_keyword(operation['face_style'] || operation['faceStyle'], style_face_modes.keys.map { |key| [key, key] }.to_h, 'style.face_style') if operation.key?('face_style') || operation.key?('faceStyle')
