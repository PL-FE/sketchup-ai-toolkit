...
require_relative 'alma_sketchup_mcp/version_adapter'
begin
  require 'sketchup.rb'
rescue LoadError
...
require_relative 'alma_sketchup_mcp/runtime_source_attestation'
AlmaSketchupMCP::RuntimeSourceAttestation.attest_model_revision!(File.join(__dir__, 'alma_sketchup_mcp')) do
  require_relative 'alma_sketchup_mcp/model_revision'
end
AlmaSketchupMCP::RuntimeSourceAttestation.attest_boolean_operations!(File.join(__dir__, 'alma_sketchup_mcp')) do
  require_relative 'alma_sketchup_mcp/boolean_operations'
end
require_relative 'alma_sketchup_mcp/structural_probe'
...
module AlmaSketchupMCP
  ...
  STATE_DIR = VersionAdapter.state_dir
  ...
  def start
    ...
    if @timer_id
      announce_bridge_state('Alma SketchUp MCP Bridge is already running.')
      return @timer_id
    end
    ...
    announce_bridge_state('Alma SketchUp MCP Bridge is running.')
    @timer_id
  end
  ...
  def stop
    ...
    @timer_id = nil
    announce_bridge_state('Alma SketchUp MCP Bridge stopped.')
  end

  def announce_bridge_state(message)
    Sketchup.status_text = message
    puts "[Alma SketchUp MCP] #{message}"
  ...
  def process_pending_requests
    ...
    Dir[File.join(QUEUE_DIR, '*.json')].sort.each do |request_path|
      ...
      response_body = begin
        ...
        VersionAdapter.assert_request!(request)
        { result: dispatch(request['method'], request['params'] || {}) }
      rescue StandardError => error
        { error: queue_error_payload(error, request) }
      end
      response_body[:bridge] = VersionAdapter.transport_identity
      ...
      begin
        ...
        warn '[Alma SketchUp MCP] Queue response persistence failed; the claimed request is retained and queue processing is paused.'
      ensure
        if response_persisted && File.exist?(processing_path)
          begin
            ...
            warn '[Alma SketchUp MCP] A persisted response still has a claimed request marker; inspect queue diagnostics before continuing.'
          end
  ...
  def get_capabilities
    {
      'name' => 'queue',
      'year_profile' => VersionAdapter.profile,
      'version_transport' => VersionAdapter.transport_identity,
      ...
      'supported_operations' => VersionAdapter.operation_support(operation_support_descriptor).select { |_op, descriptor| descriptor['status'] != 'unsupported' }.keys,
      'operation_support' => VersionAdapter.operation_support(operation_support_descriptor),
      'plugin' => {
  ...
  def reset_model
    ...
    with_atomic_model_transaction(model, 'Alma Reset Model') do
  ...
  def build_model(code)
    ...
    VersionAdapter.validate_document!(document)
  ...
  def save_model_copy(path)
    ...
    target = path.to_s.strip
    target = File.join(STATE_DIR, 'alma-sketchup-model-copy.skp') if target.empty?
    target = File.expand_path(target)
  ...
  def adopt_open_model(params = {})
    ...
    with_atomic_model_transaction(model, 'Alma Adopt Open Model') do
  ...
  unless file_loaded?(__FILE__)
    ...
    auto_item = menu.add_item('Auto-start on launch') do
      enabled = Sketchup.read_default('SketchUpAIToolkit', 'auto_start', true)
      Sketchup.write_default('SketchUpAIToolkit', 'auto_start', !enabled)
    end
    menu.set_validation_proc(auto_item) do
      Sketchup.read_default('SketchUpAIToolkit', 'auto_start', true) ? MF_CHECKED : MF_UNCHECKED
    end
    file_loaded(__FILE__)
    UI.start_timer(0.1, false) { start } if Sketchup.read_default('SketchUpAIToolkit', 'auto_start', true)
  end
end
