// Modified 2026-09-12: queue declarations reflect the selected host year.
import { PRODUCT_VERSION } from './version.mjs';
import { getVersionProfile } from './sketchup-versions.mjs';
import {
...
export function getRuntimeCapabilities(runtime = 'mock') {
  ...
  const available = OPERATION_CAPABILITIES.filter((capability) =>
    isRuntimeSupported(capability.runtime_support[runtime]) &&
    !(runtime === 'queue' && !profile.native_environments && capability.op.startsWith('environment_')));
  return {
    name: runtime,
    ...
    supported_operations: available
      .map((capability) => capability.op),
