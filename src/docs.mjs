// Modified 2026-09-12: distinguish fork validation and image-parameter workflow.
import { formatCapabilityMatrixMarkdown, getRuntimeCapabilities } from './capabilities.mjs';

export const TOOL_DOCS = `
# SketchUp AI Toolkit (multi-year alpha)

Fork-specific workflow:
- get_supported_versions reports the selected ALMA_SKETCHUP_YEAR (default 2020), registered adapters and pending host validation. The 2020 and 2026 queues are isolated. Upstream acceptance notes below are historical and do not certify this fork on either platform.
- preflight_design_brief({brief}) checks declared parameters and outputs missing / needs_confirmation / proposed_parameters. It does not analyze images, infer dimensions, verify human consent or grant live editing permission.
- For pictures, display observed evidence separately from inferred/assumed dimensions, scale and hidden geometry; ask the user to supply missing critical information or confirm proposed values before live modeling. Retain their actual confirmation; never manufacture it by setting status=confirmed.
- SketchUp 2020 supports classic materials; native PBR and environment operations are rejected before beginning a model transaction. Asset SKP files must also be readable by the selected host version.
- New installations auto-start the Ruby bridge when SketchUp loads; the Extensions > Alma SketchUp MCP menu can disable auto-start. Host-native viewport export does not capture the user's desktop.
- Use docs/installation.md, docs/workflows.md and skills/sketchup-ai-modeling/SKILL.md in this distribution. Advanced inherited feature claims require host-specific validation.
...
- queue: writes requests into ~/.sketchup-mcp/versions/<YEAR>/queue for the SketchUp Ruby plugin to execute; call get_capabilities to verify the installed plugin version, live operation support, and runtime.compatibility before long builds.
