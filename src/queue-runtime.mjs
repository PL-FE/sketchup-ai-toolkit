// Modified 2026-09-12: bind every request and response to a SketchUp year.
import fs from 'node:fs/promises';
import path from 'node:path';
import crypto from 'node:crypto';
import { defaultQueueDir, defaultResponseDir } from './paths.mjs';
import { getVersionProfile, stateDirectoryForYear, VERSION_TRANSPORT, assertVersionResponse } from './sketchup-versions.mjs';
...
export class QueueRuntime {
  constructor({
    targetYear = getVersionProfile().year,
    queueDir = process.env.ALMA_SKETCHUP_QUEUE_DIR || path.join(process.env.ALMA_SKETCHUP_STATE_DIR || stateDirectoryForYear(targetYear), 'queue'),
    responseDir = process.env.ALMA_SKETCHUP_RESPONSE_DIR || path.join(process.env.ALMA_SKETCHUP_STATE_DIR || stateDirectoryForYear(targetYear), 'responses'),
    ...
    this.targetYear = getVersionProfile(targetYear).year;
  ...
  async diagnostics({ includeFiles = false } = {}) {
    ...
    return {
      ...
      transport_version: VERSION_TRANSPORT,
      runtime: 'queue',
      state_dir: stateDir,
  ...
  async recoverOrphanResponse({ requestId, expectedResultKind, expectedClientPid } = {}) {
    ...
    return this.withExclusiveAccess(async () => {
      ...
      assertVersionResponse(response, this.targetYear);
  ...
  async callUnlocked(method, params) {
    ...
    // Verify the listener before sending any mutation. This also refuses an
    // old unversioned bridge if a custom queue path was accidentally reused.
    if (QUEUE_GUARDED_METHODS.has(method)) {
      await this.callUnlocked('get_capabilities', {});
    }
    ...
    const temporaryRequestPath = path.join(this.queueDir, `.${id}.tmp`);
    ...
    const request = { id, method, params: guardedParams, target_year: this.targetYear, transport_version: VERSION_TRANSPORT, client_pid: process.pid, created_at: new Date().toISOString() };
    ownedQueueArtifacts.add(requestPath);
    ownedQueueArtifacts.add(temporaryRequestPath);
    ...
    try {
      // Publish only complete JSON; SketchUp scans *.json on its UI timer.
      await fs.writeFile(temporaryRequestPath, `${JSON.stringify(request, null, 2)}\n`, { encoding: 'utf8', flag: 'wx', mode: 0o600 });
      await fs.rename(temporaryRequestPath, requestPath);
      ownedQueueArtifacts.delete(temporaryRequestPath);
    } catch (error) {
      await fs.rm(temporaryRequestPath, { force: true }).catch(() => {});
      ownedQueueArtifacts.delete(temporaryRequestPath);
...
function parseRecoveryResponse(raw, expectation) {
  ...
  if (Object.keys(response).length !== 2 || !Object.hasOwn(response, 'result') || !Object.hasOwn(response, 'bridge')) {
