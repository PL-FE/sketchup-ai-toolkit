#!/usr/bin/env node
// Modified 2026-09-12: year discovery and explicit design-brief preflight.
...
import { DESIGN_BRIEF_TOOL } from './design-brief.mjs';
...
const BASE_TOOL_REGISTRY = [
  DESIGN_BRIEF_TOOL,
  { name: 'get_supported_versions', description: 'Read registered SketchUp year adapters, selected target year and validation status. A configured adapter is not proof of native host testing.', inputSchema: { type: 'object', properties: {}, additionalProperties: false } },
  {
...
async function handleRequest(request, bridge) {
  if (request.method === 'initialize') {
    return respond(request.id, {
      ...
      serverInfo: { name: 'sketchup-ai-toolkit', version: PRODUCT_VERSION }
    });
  }
